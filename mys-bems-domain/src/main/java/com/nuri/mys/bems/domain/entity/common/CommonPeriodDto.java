...
@Schema(name="CommonPeriodDto")
public class CommonPeriodDto {
    @Schema(required = true, description = "입력 형식(yyyyMMddHHmmss) 12자리. 현황일 시 오늘 날짜, 통계일 시 기간 날짜", example="202209120000")
    @NotBlank
    private String dateFrom;
    @Schema(required = true, description = "입력 형식(yyyyMMddHHmmss) 12자리. 현황이나 기간 조건에 따라서 없는 경우 있음", example="202209302359")
    @NotBlank
    ...
    @Schema(required = true, description = "기간 검색 타입(시간(hour)/일(day)/월(month)/년(year)/기간(period))", example="month")
    @NotBlank
